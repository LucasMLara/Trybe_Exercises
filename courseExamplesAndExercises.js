...
db.movies.updateOne({title:/godzilla/i},
...
// db.movies.find({ ratings: {$gt:100, $lt: 105}},
// {title:1, ratings:1, _id:0});
...
//Checado no gabarito. Pensei em usar AND para acumular os requisitos da query. mas podemos por todos os criterios dentro do objeto do elemMatch, conforme query acima. nesse caso só o rating 99 satisfaz essa condição.
...
db.movies.find(
{$or:[
  {category:{$all:[/sci-fi/i]}},{ratings:{$gt:199}}
]},  
    {title:1, ratings:1, category:1, _id:0});

    // MISSING $OR!! GET MORE CONFIDENCE! CHECK LAST CLASSES!
...
// primeira tentativa foi sem por as chaves entre os elementos do array $and; . após correção. retorno de array vazio.
// lembrar dos operadores das aulas passadas.  o conhecimento é acumulativo.
...
//   $and:[
//     {ratings:{ $size:4}},
//     {category:{ $all:[/adventure/i, /family/i]}},
//     {imdbRating:{ $gte:7}}
//   ]
// });
